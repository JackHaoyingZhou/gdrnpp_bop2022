...
p = {
    ...
    "correct_th": {
        ...
        "te": [5.0],  # [cm].
        "proj": [5.0],  # [px].
        # sym aware re/te/rete/proj
        "reteS": [5.0, 5.0],  # [deg, cm].
        "reS": [5.0],  # [deg].
        "teS": [5.0],  # [cm].
        "projS": [5.0],  # [px].
        #
    ...
    "error_dir_paths": [r"/path/to/calculated/errors"],
    ...
    "error_tpath": osp.join("{eval_path}", "{error_dir_path}", "errors_{scene_id:06d}.json"),
    # Template of path to the output file with established matches and calculated
    # scores.
    "out_matches_tpath": osp.join("{eval_path}", "{error_dir_path}", "matches_{score_sign}.json"),
    "out_scores_tpath": osp.join("{eval_path}", "{error_dir_path}", "scores_{score_sign}.json"),
}
################################################################################
...
for error_dir_path in p["error_dir_paths"]:
    ...
    misc.log("error: {}, method: {}, dataset: {} {}".format(err_type, method, dataset, score_sign))
