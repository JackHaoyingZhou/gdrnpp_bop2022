...
class AMBF_SUTURING_Dataset(object):
    """ambf_suturing bop test splits."""

    def __init__(self, data_cfg):
        """
        Set with_depth and with_masks default to True,
        and decide whether to load them into dataloader/network later with_masks:
        """
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.ambf_suturing.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.ambf_suturing.obj2id[obj_name]:06d}.ply",
                ),
...
def get_ambf_suturing_metadata(obj_names, ref_key):
...
#             )
#         ]
#         if name not in SPLITS_AMBF:
#             SPLITS_AMBF[name] = dict(
#                 name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_AMBF:
        used_cfg = SPLITS_AMBF[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, AMBF_SUTURING_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="ambf",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_ambf_suturing_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_AMBF.keys())


#### tests ###############################################
def test_vis():
    dset_name ="ambf_suturing" # sys.argv[1]
...
if __name__ == "__main__":
    ...
    register_with_name_cfg("ambf_suturing") #sys.argv[1]
